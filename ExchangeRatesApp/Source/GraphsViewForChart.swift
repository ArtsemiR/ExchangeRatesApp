...
struct GraphsViewForChart: View {
    ...
    var body: some View {
        GeometryReader { geometry in
            ZStack {
                YTickerView(chart: self.chart,
                            rangeTime: self.rangeTime,
                            colorYAxis: Color("ColorTitle"),
                            colorYMark: Color.primary)
                
                GraphsForChart(chart: self.chart,
                               rangeTime: self.rangeTime,
                               lineWidth : 2)
    ...
    static var previews: some View {
        NavigationView {
            GraphsViewForChart(
                ...
                rangeTime: 0..<(periodRatesData[0].lines[0].points.count - 1))
