...
struct YTickerView : View {
    ...
    var rangeY: Range<Int>? {
        let rangeY = rangeOfRanges(chart.lines.filter { !$0.isHidden }
            .map { ($0.points[rangeTime].min() ?? 0)..<($0.points[rangeTime].max() ?? 0) })
    ...
    var body: some View {
        GeometryReader { geometry in
            VStack (spacing: 0) {
                ForEach(self.calcScale(height: geometry.size.height).marks.reversed(),
                        id:\.self) { mark in
                            YMarkView(yValue: mark,
                                      colorYAxis: self.colorYAxis,
                                      colorYMark: self.colorYMark)
                } // ForEach
    ...
    private func calcScale(height: CGFloat) -> (scaleY: CGFloat, stepYHeight: CGFloat, marks: [Int]) {
        ...
        let stepYHeight: CGFloat = height / CGFloat((0..<self.estimatedMarksNumber).distance - 1)
        let stepYValue: CGFloat = stepYHeight / scaleY
        var marks = [Int]()

        for i in 0..<self.estimatedMarksNumber {
