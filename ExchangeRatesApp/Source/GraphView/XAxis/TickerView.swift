...
struct TickerView : View {
    ...
    var body: some View {
        GeometryReader { geometry in
            ScrollView(.horizontal, showsIndicators: false) {
                HStack (spacing: 0) {
                    ForEach(self.calcScale(widthRange: geometry.size.width).indexes,
                            id: \.self) { indexMark in
                                TimeMarkView(index: indexMark,
                                             xTime: self.chart.xTime,
    ...
    private func calcScale(widthRange: CGFloat) -> (scaleTime :CGFloat,step: CGFloat, indexes: [Int]) {
        ...
        let stepIndex = Int(estimatedStepMark / scaleTime + 0.1)
